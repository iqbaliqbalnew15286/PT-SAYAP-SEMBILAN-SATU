...
@section('content')
    ...
    <div class="bg-[#F4F7FA] min-h-screen font-['Poppins'] text-[#161f36] overflow-x-hidden" x-data="{
        activeSector: 'all',
        search: '',
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
                    @forelse ($partners as $partner)
                        <div x-show="isVisible('{{ $partner->sector }}', '{{ $partner->name }}')"
                            ...
                            <div class="w-full h-40 flex items-center justify-center mb-8 bg-gray-50 rounded-[2rem] p-6 group-hover:bg-white transition-colors duration-500 relative overflow-hidden">
                                ...
                                @if ($partner->logo)
                                    @php
                                        // Logika: Jika string mengandung http, berarti itu link Cloudinary/External.
                                        // Jika tidak, berarti path lokal storage.
                                        $logoUrl = Str::contains($partner->logo, ['http://', 'https://'])
                                                   ? $partner->logo
                    ...
                    @empty
                        <div class="col-span-full py-24 text-center">
                             <h3 class="text-lg font-black text-gray-400 uppercase tracking-[0.3em]">Belum Ada Data Mitra</h3>
                        </div>
                    @endforelse
                </div>

                {{-- Empty State (Search) --}}
                <div x-show="search !== ''" class="py-24 text-center" x-cloak>
                    ...
                    <h3 class="text-lg font-black text-gray-400 uppercase tracking-[0.3em]">Pencarian Tidak Ditemukan</h3>
                </div>
            </div>
        </section>
