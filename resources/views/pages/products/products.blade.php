...
@section('content')
    ...
    @php
        ...
        $heroImage1 = $items->skip(0)->first() ? asset('storage/' . $items->skip(0)->first()->image) : 'https://images.unsplash.com/photo-1544380904-c686119ec4f5?q=80&w=2000';
        $heroImage2 = $items->skip(1)->first() ? asset('storage/' . $items->skip(1)->first()->image) : 'https://images.unsplash.com/photo-1581092160562-40aa08e78837?q=80&w=2000';
        ...
        <div class="max-w-7xl mx-auto px-6">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
                ...
                <div class="text-center lg:text-left">
                    ...
                    <h2 class="text-3xl md:text-5xl font-extrabold text-[#161f36] heading-tight uppercase mb-6">Infrastruktur <br> Tanpa Batas</h2>
                    <p class="text-gray-500 leading-relaxed mb-8 font-medium">Kami menyediakan perangkat keras dan jasa teknis profesional untuk memastikan jaringan telekomunikasi Anda tetap stabil dan berkembang mengikuti zaman.</p>
                    <ul class="space-y-4">
        ...
        <div class="max-w-7xl mx-auto px-6">
            <div class="flex flex-col md:flex-row justify-between items-end mb-16 gap-8">
                ...
                <div class="flex gap-2 bg-gray-100 p-1.5 rounded-full">
                    <button @click="activeTab = 'all'" :class="activeTab === 'all' ? 'bg-orange-main text-white shadow-md' : 'text-gray-500 hover:text-navy'" class="px-8 py-2.5 rounded-full text-[11px] font-black uppercase transition-all tracking-widest">Semua</button>
                    <button @click="activeTab = 'barang'" :class="activeTab === 'barang' ? 'bg-orange-main text-white shadow-md' : 'text-gray-500 hover:text-navy'" class="px-8 py-2.5 rounded-full text-[11px] font-black uppercase transition-all tracking-widest">Barang</button>
                    <button @click="activeTab = 'jasa'" :class="activeTab === 'jasa' ? 'bg-orange-main text-white shadow-md' : 'text-gray-500 hover:text-navy'" class="px-8 py-2.5 rounded-full text-[11px] font-black uppercase transition-all tracking-widest">Jasa</button>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 lg:gap-14">
                ...
                <div x-show="activeTab === 'all' || activeTab === '{{ $item->type }}'"
                    ...
                    <div class="relative h-72 overflow-hidden">
                        <img src="{{ asset('storage/' . $item->image) }}" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-1000">
                        <div class="absolute top-6 left-6 bg-white/95 backdrop-blur-sm px-4 py-1.5 rounded-xl text-[9px] font-black uppercase text-navy shadow-sm tracking-[0.1em]">
                            {{ $item->type }}
                        </div>
                    </div>

                    <div class="p-8">
                        <h3 class="text-xl font-extrabold text-navy mb-3 group-hover:text-orange-main transition-colors duration-300">{{ $item->name }}</h3>
                        ...
                        <div class="flex justify-between items-center pt-6 border-t border-gray-100">
                            <span class="text-navy font-black text-sm tracking-tight">Rp {{ number_format($item->price, 0, ',', '.') }}</span>
                            <a href="{{ route('product.show', $item->id) }}" class="text-orange-main font-black text-[10px] uppercase tracking-[0.15em] flex items-center gap-2 hover:gap-3 transition-all">
    ...
    <section class="pb-24 px-6">
        <div class="max-w-7xl mx-auto bg-navy rounded-[4rem] p-12 md:p-24 text-center relative overflow-hidden shadow-2xl shadow-navy/30">
            ...
            <div class="relative z-10">
                <h2 class="text-3xl md:text-6xl font-extrabold text-white mb-8 uppercase heading-tight">Elevating <br> <span class="text-orange-main">Connectivity</span></h2>
                <p class="text-gray-400 mb-14 max-w-xl mx-auto font-medium leading-relaxed opacity-80">Rencanakan kebutuhan infrastruktur telekomunikasi Anda bersama mitra konstruksi terpercaya.</p>
                <a href="{{ route('booking') }}" class="bg-orange-main text-white px-16 py-5 rounded-[2rem] font-black text-xs uppercase tracking-[0.2em] hover:bg-white hover:text-navy transition-all duration-300 shadow-xl shadow-orange-600/20 active:scale-95">Book Project Now</a>
            </div>
        </div>
    </section>
@endsection
