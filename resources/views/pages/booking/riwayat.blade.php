...
<body class="bg-[#F8FAFC] text-slate-700" x-data="{ sidebarOpen: true }">
        ...
        <aside :class="sidebarOpen ? 'w-72' : 'w-20'"
            ...
            <nav class="flex-1 px-4 py-6 space-y-2 overflow-y-auto no-scrollbar">
                ...
                <a href="{{ route('booking.riwayat') }}"
                    class="flex items-center px-4 py-3 bg-orange-50 text-orange-600 rounded-xl group transition-all"
                    :class="!sidebarOpen && 'justify-center'">
            ...
            <div class="p-4 border-t border-slate-50">
                <form action="{{ route('booking.logout') }}" method="POST">
                    @csrf
                    <button type="submit"
                        class="w-full flex items-center px-4 py-3 text-sm font-bold text-red-500 hover:bg-red-50 rounded-xl transition-all"
                                    ...
                                    <tr class="hover:bg-slate-50/50 transition-colors">
                                        <td class="px-6 py-5">
                                            <p class="font-bold text-slate-800 text-sm capitalize">
                                                {{ $booking->note ?? ($booking->services ?? 'Layanan Tidak Diketahui') }}
                                            </p>
                                            <p class="text-[10px] text-slate-400 font-medium mt-0.5 tracking-wider">
                                        ...
                                        <td class="px-6 py-5">
                                            <div class="flex items-center text-sm text-slate-700 font-semibold">
                                                <i class="fa-regular fa-calendar mr-2 text-orange-500"></i>
                                                {{ \Carbon\Carbon::parse($booking->date)->format('d M Y') }}
                                            </div>
                                            <p class="text-[11px] text-slate-400 font-medium ml-6 mt-1">
                                                {{ $booking->time }} WIB
                                            </p>
                                        </td>
