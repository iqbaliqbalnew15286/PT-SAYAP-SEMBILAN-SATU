    ...
    <div class="flex min-h-screen">
        <aside
            ...
            <nav class="flex-1 px-4 py-6 space-y-2 overflow-y-auto no-scrollbar">
                ...
                <a href="{{ route('booking.riwayat') }}" class="flex items-center px-4 py-3 text-slate-500 rounded-xl hover:bg-slate-50 group transition-all" :class="!sidebarOpen && 'justify-center'">
            ...
            <div class="p-4 md:p-8 max-w-6xl mx-auto w-full flex-1">
                ...
                <div class="mt-12 bg-slate-900 rounded-[32px] p-6 md:p-10 flex flex-col md:flex-row items-center justify-between gap-8 shadow-2xl">
                    ...
                    <div class="flex gap-4 w-full md:w-auto">
                        ...
                        <button x-show="step === 2" @click="submitBooking()" :disabled="!bookingDate || !bookingTime || loading" class="flex-1 md:px-12 py-4 bg-green-500 text-white font-bold text-sm rounded-2xl hover:bg-green-600 transition-all shadow-xl shadow-green-500/20 disabled:opacity-50">
                            <span x-show="!loading">Kirim WhatsApp <i class="fa-brands fa-whatsapp ml-2 text-lg"></i></span>
    ...
    <script>
        function bookingApp() {
            return {
                ...
                // FUNGSI BARU: Simpan data ke Database lalu buka WhatsApp
                async submitBooking() {
                    this.loading = true;
                    ...
                    try {
                        // Kirim data ke Controller menggunakan Fetch API (Laravel Route)
                        const response = await fetch("{{ route('booking.riwayat.store') }}", { // Pastikan route ini ada di web.php
                            method: "POST",
                            headers: {
                        ...
                        if (response.ok) {
                            // Jika berhasil simpan di database, buka WhatsApp
                            const phone = "6289502669582";
                            const text = `*PESANAN BOOKING BARU*\n\n*Pelanggan:* {{ auth()->user()->name }}\n*Layanan:* ${data.services}\n*Total:* ${this.formatRupiah(data.total_price)}\n*Jadwal:* ${data.date} (${data.time} WIB)`;

                            window.open(`https://wa.me/${phone}?text=${encodeURIComponent(text)}`, '_blank');

                            // Redirect ke halaman riwayat agar user lihat booking barunya
